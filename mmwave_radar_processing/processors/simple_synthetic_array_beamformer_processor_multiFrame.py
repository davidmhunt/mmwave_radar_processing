...
class SyntheticArrayBeamformerProcessor(_Processor):
    ...
    def __init__(
        ...
        #history tracking
        self.history_avg_vel:np.ndarray = np.zeros(shape=num_frames,dtype=float)
    ...
    def _compute_key_radar_parameters(self):
        ...
        #compute the start time for each chirp (indexed by [frame,chirp])
        all_chirp_start_times_us = np.arange(self.chirps_per_frame) * self.chirp_period_us
        valid_chirp_start_times = all_chirp_start_times_us[self.valid_chirps_mask]

        self.chirp_start_times_us = None
