...
class _BeamformerProcessor(_Processor):
    ...
    def __init__(
            ...
            el_angle_bins_rad=np.array([0])) -> None:
        ...
        #array geometry
        self.array_geometry:np.ndarray = np.empty(shape=0) #indexed by rx index
        ...
        self.beamformed_resp:np.ndarray = None #indexed by [sample,theta, phi]

        #mesh grids for spherical plotting - indexed by (theta, phi)
    ...
    def _compute_mesh_grids(self):
        ...
        self.thetas, self.phis = np.meshgrid(
            self.az_angle_bins_rad, self.el_angle_bins_rad,
            indexing="ij"
        )

        return
    
    def _generate_array_geometry(self):
        
        #assume that the rx position origin is at Rx0
        if self.array_geometry == "standard":
            NotImplementedError("Standard array geometry generation not implemented yet.")
        elif self.array_geometry == "ods":
