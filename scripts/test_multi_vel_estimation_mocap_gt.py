...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate aggregated Radar and Flow velocity estimates against MOCAP.")
    parser.add_argument(
        "--config-name",
        type=str,
        default="IcaRAus_multi_dataset_velocity_analysis_config.yaml",
    ...
    parser.add_argument(
        "--results-dir",
        type=str,
        default="IcaRAus_multi_vel_mocap_results",
...
def main():
    ...
    analyzer_radar.analyze(radar_vel[start_idx:end_idx], mocap_vel[start_idx:end_idx], error_method)
    analyzer_flow.analyze(flow_vel[start_idx:end_idx], mocap_vel[start_idx:end_idx], error_method)
    ...
    raw_df = pd.DataFrame({
        ...
        'mocap_vx': mocap_vel[start_idx:end_idx, 0], 'mocap_vy': mocap_vel[start_idx:end_idx, 1], 'mocap_vz': mocap_vel[start_idx:end_idx, 2],
    ...
    for i, component in enumerate(components):
        plotter.plot_comparison_time_series(flow_vel[start_idx:end_idx, i], mocap_vel[start_idx:end_idx, i], ax=axs_comp[i, 0], title=f"Flow vs MOCAP: {component} Velocity", ylabel="m/s", est_label="Flow", gt_label="MOCAP", est_color=colors[i], show=False)
        plotter.plot_comparison_time_series(radar_vel[start_idx:end_idx, i], mocap_vel[start_idx:end_idx, i], ax=axs_comp[i, 1], title=f"Radar vs MOCAP: {component} Velocity", ylabel="m/s", est_label="Radar", gt_label="MOCAP", est_color=colors[i], show=False)
    plt.tight_layout()
    fig_comp.savefig(os.path.join(results_path, "velocity_comparison_mocap.png"))
    plt.close(fig_comp)

    # 2. Combined Error Summary Plot (2x2)
    # Columns: Flow, Radar | Rows: Time Series, CDF
    fig_sum, axs_sum = plt.subplots(2, 2, figsize=(18, 12), gridspec_kw={'height_ratios': [2, 1]})
    
    # Flow Summary (Col 0)
    plotter.plot_velocity_analysis_summary(
        analyzer_flow.get_x_errors(), analyzer_flow.get_y_errors(), 
        analyzer_flow.get_z_errors(), analyzer_flow.get_norm_errors(), 
        axs=axs_sum[:, 0], show=False
    )
    axs_sum[0, 0].set_title("Flow: Velocity Estimation Errors Over Time", fontsize=15)
    
    # Radar Summary (Col 1)
    plotter.plot_velocity_analysis_summary(
        analyzer_radar.get_x_errors(), analyzer_radar.get_y_errors(), 
        analyzer_radar.get_z_errors(), analyzer_radar.get_norm_errors(), 
        axs=axs_sum[:, 1], show=False
    )
    ...
    # Columns: Flow, Radar | Rows: X, Y, Z
    fig_hist, axs_hist = plt.subplots(3, 2, figsize=(15, 12))
    
    # Flow Histograms (Col 0)
    plotter.plot_error_histograms(
        analyzer_flow.get_x_errors(), analyzer_flow.get_y_errors(), 
        analyzer_flow.get_z_errors(), axs=axs_hist[:, 0], show=False
    )
    # Add column header titles manually for clarity
    axs_hist[0, 0].set_title(f"Flow: X Velocity Error Distribution\nMean: {np.mean(analyzer_flow.get_x_errors()):.4f}", fontsize=12)
    
    # Radar Histograms (Col 1)
    plotter.plot_error_histograms(
        analyzer_radar.get_x_errors(), analyzer_radar.get_y_errors(), 
        analyzer_radar.get_z_errors(), axs=axs_hist[:, 1], show=False
    )
